...
class KeymappingsStore {
	...
	getMapping = (keyName) => {
		const selectedSet = this.sets[this.selectedSetIndex]
        store.canvas[store.selected_y][store.selected_x] = [...this.sets[this.selectedSetIndex][keyName], store.glyphOffsetX, store.glyphFontSizeModifier, store.rotationAmount, store.flipGlyph, store.glyphInvertedColor];
